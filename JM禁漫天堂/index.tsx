...
const SETTINGS_KEY = "jmcomic_scripting_settings_v2"
...
const HOSTS: JmHost[] = [
  ...
  { id: "jmcomic-zzz.one",    label: "jmcomic-zzz.one",     url: "https://jmcomic-zzz.one",    group: "东南亚路线", note: "东南亚建议使用" },
...
function BrowseView(props: {
  ...
  const openCategories = (): void => {
    const url = joinUrl(getBaseUrl(props.settings), "/albums")
    props.openUrl(url)
  }
    ...
    <VStack spacing={20} padding={{ horizontal: 16, top: 16, bottom: 40 }} alignment="leading">
      ...
      <GlassCard>
        ...
        <Text font="subheadline" foregroundStyle={theme.textSecondary}>
          从首页推荐与分类列表发现内容；阅读直接在 Safari 内嵌浏览器中打开。
        </Text>
        <HStack spacing={12}>
        ...
        <HStack spacing={12}>
          ...
          <ActionButton title="打开官网" systemImage="safari" action={openCategories} />
        </HStack>
      </GlassCard>
    ...
    <VStack spacing={20} padding={{ horizontal: 16, top: 16, bottom: 40 }} alignment="leading">
      ...
      <GlassCard>
        ...
        <Text font="caption" foregroundStyle={theme.textTertiary}>
          使用 Safari 内嵌浏览器打开禁漫天堂网站。首次遇到 Cloudflare 验证请手动完成，之后通常会保持通过状态。本脚本不绕过任何验证机制。
        </Text>
      </GlassCard>
    </VStack>
  </ScrollView>
